...
`psych/final/index.html` built on the newer jump-nav/readiness-ring hub layout (same as `exam3/index.html`), not the older layout `exam1`/`exam2` still use. Single topic-pair card so far (Neurocognitive KC + DD both live); more topic-pairs get appended to the `EXAM.topics` array as chapters are built. No prof-questions banner yet since no professor-supplied question set exists for the final the way Exam 3 had one; can add later if that surfaces. Parent list (`psych/index.html`) flipped Final to `ready: true` with `href: "final/index.html"` so Tom can navigate and test as the hub grows, topics text flagged "(in progress)".

Next: repeat the KC+DD pattern for Ch 22, 23, 20, 21, then a cumulative simulator once all five topics exist.
