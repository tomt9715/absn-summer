...
Next: Section 4 (Schizophrenia & Psychosis, source: exam2 Schizophrenia -- command hallucinations, EPS spectrum, clozapine monitoring, NMS), then continue through Mood/Lithium/Suicide, Substance Use/Withdrawal, then the Exam 3 rapid-refresher topics (Anxiety, OCD, Trauma, Personality, Abuse, Anger) one at a time, appending to the same data file each time using `Filesystem:edit_file`.
