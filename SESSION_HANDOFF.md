...
**Cumulative Review section (Aug 7, 2026)** — Tom asked whether Exam 1-3's existing KC/DD banks actually cover what Unit 6 asks for, and if so, to link them straight into the Final hub instead of rebuilding. Spot-checked three full data files by reading them in full (not just topic titles): `exam1/data/theories-kc.js` + `theories-dd.js` (defense mechanisms), `exam2/data/mood-kc.js` (lithium/suicide/mania), `exam2/data/addiction-kc.js` (withdrawal/DTs/MAT). All three matched or exceeded what Unit 6 asks for. One minor gap found: reaction formation, compensation, and identification aren't explicitly named as terms in the theories bank (splitting, rationalization, repression, sublimation, projection, regression, denial, displacement all are, plus SASH) -- flagged to Tom as a small, lower-yield gap rather than silently patched, since editing an Exam 1 file wasn't part of the ask. Did not exhaustively read every remaining file (comm, mse, schizophrenia, all six Exam 3 topics) given the consistent depth found in the three that were checked, all built in the same session/style.

Added a `.cumulative-banner` section to `psych/final/index.html`, positioned after the six topic-pairs and before the donation footer, with its own jump-nav chip (`#cumulative-review`). Reuses the `.prof-card`-style link-card pattern already established on the Exam 2/3 hubs (adapted inline since Final's stylesheet doesn't share that CSS). Twelve cards, each linking to `../examN/index.html#topic-slug` on the relevant Exam hub rather than duplicating any content: Exam 1 (Therapeutic Communication, Theories & Defense Mechanisms, MSE), Exam 2 (Schizophrenia & Psychosis, Mood Disorders & Suicide, Addiction & Substance Use), Exam 3 (Anxiety, OCD, Trauma, Personality, Abuse, Anger). All twelve anchor IDs verified against the actual `id="topic-*"` attributes in each exam's index.html before writing. No new question banks, no new keys, no changes to `ALL_KEYS` or the readiness ring, since these link out rather than adding new local content.
